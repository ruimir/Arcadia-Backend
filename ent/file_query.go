...
func (fq *FileQuery) QueryRom() *RomQuery {
	query := &RomQuery{config: fq.config}
...
func (fq *FileQuery) WithRom(opts ...func(*RomQuery)) *FileQuery {
	query := &RomQuery{config: fq.config}
...
func (fq *FileQuery) sqlAll(ctx context.Context) ([]*File, error) {
	...
	if query := fq.withRom; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*File)
		for i := range nodes {
			if nodes[i].file_rom == nil {
				continue
			}
			fk := *nodes[i].file_rom
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "file_rom" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Rom = n
			}
