...
func (rq *ReleaseQuery) QueryGame() *GameQuery {
...
func (rq *ReleaseQuery) WithGame(opts ...func(*GameQuery)) *ReleaseQuery {
	query := &GameQuery{config: rq.config}
...
func (rq *ReleaseQuery) sqlAll(ctx context.Context) ([]*Release, error) {
	...
	if query := rq.withGame; query != nil {
		...
		for i := range nodes {
			if nodes[i].game_releases == nil {
				continue
			}
			fk := *nodes[i].game_releases
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "game_releases" returned %v`, n.ID)
			}
			for i := range nodes {
